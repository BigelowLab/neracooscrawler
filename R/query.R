...
MODISA_query <- function(
   top = "http://www.neracoos.org/thredds/catalog/GMRI/SATELLITE/NASA/MODISA_NESHELF/catalog.xml",
   platform = 'MODISA', 
   product = 'L3SMI',
   ...
   get_all <- function(Product, year, day, greplargs = NULL, verbose = FALSE) {
   ...
   if (what == "most_recent"){
      ...
      for (i in seq_along(yd)){
         y <- names(yd)[i]
         R[[y]] <- get_all(Product, y, yd[[i]], greplargs = greplargs, verbose = verbose)
      }
   ...
   if (verbose){
      cat(sprintf("MODISA_query: retrieved %i records\n", length(R)))
   }
   invisible(R)   
}
